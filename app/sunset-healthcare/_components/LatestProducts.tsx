    ...
    >
      ...
      <div className="container relative z-10">
        <div className="text-center">
          <h3 className="font-bold mb-3 text-[16px] uppercase tracking-widest text-white/80">Latest Products</h3>
          <h2 className="text-[25px] md:text-[30px] font-medium mb-6 leading-[1.2]">
            A Trusted Leader in HME Solutions Since 2004
          </h2>
          <p className="text-[16px] text-white/90 mb-12 max-w-5xl mx-auto leading-[1.7] px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {products.map((prod, idx) => (
            <div key={idx} className="bg-[#78828b] rounded-xl overflow-hidden flex flex-col text-left hover:-translate-y-1 transition-transform border border-white/10">
              <div className="w-full flex items-center justify-center">
                <img src={`/sunset-healthcare/home${prod.image}`} alt={prod.name} className="w-full h-full aspect-squire object-cover bg-[#fdf2f2]" />
              </div>
              <div className="p-5">
                <p className="text-[14px] text-white/70 mb-1">{prod.category}</p>
                <h4 className="font-medium text-white text-[18px] leading-snug pr-2">{prod.name}</h4>
              </div>
            </div>
          ))}
        </div>
