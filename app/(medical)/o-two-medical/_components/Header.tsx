  ...
  return (
    ...
    >
      ...
      >
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`relative py-1 text-white hover:text-white transition-all font-semibold font-sans tracking-wide group ${isActive ? "text-white" : "text-white/75"
                  }`}
              >
                {link.name}
                <span
                  className={`absolute bottom-0 left-0 h-[2.5px] bg-white rounded-full transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"
                    }`}
                />
              </Link>
            );
        ...
        {/* Search */}
        <div className="hidden lg:flex items-center bg-white rounded-full pl-5 pr-10 py-2.5 w-[260px] relative shadow-inner focus-within:ring-2 focus-within:ring-[#00e5ff] focus-within:ring-offset-2 focus-within:ring-offset-[#0091d5] transition-all">
          <input
            type="text"
      ...
      {open && (
        <div className="lg:hidden absolute top-full left-[2.5%] w-[95%] bg-[#0091d5] border border-white/10 rounded-3xl mt-2.5 flex flex-col gap-4 text-white text-[17px] p-6 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-300">
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`py-1.5 font-semibold font-sans tracking-wide transition-all ${isActive ? "text-[#00e5ff] border-l-2 border-[#00e5ff] pl-3" : "text-white/90 hover:text-[#00e5ff]"
                  }`}
              >
                {link.name}
              </Link>
          ...
          <div className="flex items-center bg-white rounded-full pl-4 pr-10 py-2.5 mt-2 shadow-inner relative">
            ...
            />
            <button className="absolute right-0.5 flex items-center justify-center rounded-r-full w-10 h-[calc(100%-4px)] bg-[#0091d5] text-white cursor-pointer hover:bg-[#007bb8] transition-all">
