...
export default function Typography({
  ...
  const variantStyles = {
    ...
    body: "text-[14px] sm:text-[15px] md:text-[16px] lg:text-[16px] xl:text-[18px] leading-[1.65]",
  ...
  const fontClass = isHeading ? "font-teko  tracking-wide" : "font-exo";
  ...
  const computedWeight = weight || defaultWeights[variant] || "normal";

  const combinedClasses = `
    ${fontClass}
      ${variantStyles[variant]}
      ${colorStyles[color as keyof typeof colorStyles] || ""}
      ${weightStyles[computedWeight]}
